...
def fit_fun(X, dim, x_max):  # 适应函数
    Y = np.array([i for i in range(dim)])  # 测试的比较值
    num = np.int64(np.array(X) > (x_max * 1.0 / 2)).sum()
    total = (np.int64(np.array(X) > (x_max * 1.0 / 2)) * Y).sum()
...
class Particle:
    ...
    def __init__(self, x_max, max_vel, dim):
        ...
        self.__fitnessValue, num = fit_fun(self.__pos, dim, x_max)  # 适应度函数值
...
class PSO:
    def __init__(self, dim, size, iter_num, x_max, max_vel, itemLimit, best_fitness_value=float('Inf'), C1=2, C2=2, W=1):
        self.C1 = C1
        self.C2 = C2
        self.W = W
        self.dim = dim  # 粒子的维度
        self.num = dim  # 初始选中维度数量
        self.size = size  # 粒子个数
        self.iter_num = iter_num  # 迭代次数
        self.itemLimit = itemLimit # 迭代值域
        self.x_max = x_max
    ...
    # 更新位置
    def update_pos(self, part, dim, x_max):
        for i in range(self.dim):
            pos_value = part.get_pos()[i] + part.get_vel()[i]
            part.set_pos(i, pos_value)
        value, self.num = fit_fun(part.get_pos(), dim, x_max)
    ...
    def update(self, dim, x_max):
        for i in range(self.iter_num):
            for part in self.Particle_list:
                self.update_vel(part)  # 更新速度
                self.update_pos(part, dim, x_max)  # 更新位置
            self.fitness_val_list.append(self.get_bestFitnessValue())  # 每次迭代完把当前的最优适应度存到列表
            print(self.num)
            if self.num < dim * self.itemLimit:
                break
